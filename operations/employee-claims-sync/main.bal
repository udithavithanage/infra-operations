...
public function main() returns error? {
    ...
    foreach employee:Employee employee in employees {
        ...
        if userResult.length() == 0 {
            log:printWarn(string `User with employee ID ${employee.employeeId} does not exist in Asgardeo. Skipping...`);
            count += 1;
            continue;
        }
        ...
        if jobTitleNeedsUpdate || profileUrlNeedsUpdate {
            ...
            if updatedUser is error {
                log:printError(string `Failed to update user ${user.id} in Asgardeo.`, updatedUser);
                updateFailureCount += 1;
            } else {
                log:printDebug(string `Successfully updated user ${user.id} in Asgardeo.`);
            }
        }
        count += 1;
    }
